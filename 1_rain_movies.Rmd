...
basemap2 <- tm_shape(co) + 
  ...
  tm_shape(co_elev) +
    tm_raster(style='cont',
              palette = 'Greens',
              title='Elevation (m)') 
county_map <- tm_shape(co_counties)+
...
saveVideo({
  ...
  if(nrow(sub_sf) > 0){
  finalmap <- basemap2+ county_map + tm_shape(sub_sf) + 
...
I was unable to create/play the video because I am missing "ffmpeg" (I am in R 3.5.1). I was unable to get the program installed on my computer. I never got the videos that you created running properly from the same issue. I tried testing this on my work computer (R 3.6.1), but I got the same error. Therefore, the script I wrote wasn't capable of producing a MP4 file. 